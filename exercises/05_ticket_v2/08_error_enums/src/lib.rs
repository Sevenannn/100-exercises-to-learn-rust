...
#[derive(Debug)]
enum TicketNewError {
    TitleInvalid { value: String },
    DescriptionInvalid { value: String },
}
...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    match Ticket::new(title.clone(), description, status.clone()) {
        Ok(ticket) => ticket,
        Err(e) => match e {
            TicketNewError::DescriptionInvalid { value } => {
                Ticket::new(title, "Description not provided".into(), status).unwrap()
            }
            TicketNewError::TitleInvalid { value } => {
                panic!("{:?}", value)
            }
        },
    }
    ...
    pub fn new(
        ...
        if title.is_empty() {
            return Err(TicketNewError::TitleInvalid {
                value: "Title cannot be empty".to_string(),
            });
        }
        if title.len() > 50 {
            return Err(TicketNewError::TitleInvalid {
                value: "Title cannot be longer than 50 bytes".to_string(),
            });
        }
        if description.is_empty() {
            return Err(TicketNewError::DescriptionInvalid {
                value: "Description cannot be empty".to_string(),
            });
        }
        if description.len() > 500 {
            return Err(TicketNewError::DescriptionInvalid {
                value: "Description cannot be longer than 500 bytes".to_string(),
            });
        }
